...
<div class="top-menu">
    ...
    <a href="<c:url value='/searchsupplier.do'/>" class="btn">仕入先名で検索</a>
...
<form method="post">
  ...
  <div id="idsContainer"></div> <!-- JavaScriptで選択IDをhiddenに格納 -->
  ...
  <div class="toolbar">
    ...
    <button class="btn"
            ...
            name="to" value="X"
            onclick="return prepStatus() && confirm('選択項目を中断しますか？');">選択項目 中断</button>

    <!-- 表示状態(A/X)を一括変更: 再開(A)にする -->
    <button class="btn"
            formaction="${pageContext.request.contextPath}/status/bulk.do"
            name="to" value="A"
            onclick="return prepStatus();">選択項目 再開</button>

    <!-- チェックを入れると「中断(X)」も表示対象に含める -->
    <label style="margin-left:12px;">
      <input type="checkbox"
             onchange="location.href='?includeHidden='+(this.checked?'1':'0');"
        <c:if test="${param.includeHidden=='1' || includeHidden}">checked</c:if> /> 中断を含む
    </label>
  </div>
  ...
  <table>
    <tr>
        <th><input type="checkbox" onclick="toggleAll(this)"></th> <!-- 全選択 -->
        <th>仕入先ID</th>
        <th>仕入先名</th>
        <th>連絡先</th>
        <th>住所</th>
    ...
    <c:forEach var="supplier" items="${supplierList}">
      ...
      <c:if test="${param.includeHidden=='1' || includeHidden || empty supplier.row_status || supplier.row_status=='A'}">
        <tr class="<c:if test='${supplier.row_status=="X"}'>muted</c:if>">
            ...
            <td>
              <c:choose>
                <c:when test="${empty supplier.row_status || supplier.row_status=='A'}"><span class="badge-ok">進行中</span></c:when>
                <c:otherwise><span class="badge-stop">中断</span></c:otherwise>
              </c:choose>
            </td>
        </tr>
      </c:if>
    ...
    <c:if test="${empty supplierList}">
        <tr><td colspan="7">登録された仕入先はありません。</td></tr>
    </c:if>
  </table>
</form>
