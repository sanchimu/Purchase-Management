...
<!-- 검색 폼 -->
<form action="searchProducts.do" method="get" style="width:80%; margin:20px auto; text-align:center;"><!-- submit 시 실행된 action과 데이터 전달 방식 (get,post)세팅 -->
	<!-- 아래는 검색창인데 검색 후 검색했던 값을 그대로 유지하기 위해 value에 param.~~를 사용했음 -->
    ...
    <select name="category" style="margin-right:10px; padding:4px;">
        ...
        <c:forEach items="${categoryList}" var="cat"><!-- foreach로 catogoryList에 있는 값이 다 나올때까지 반복되고 그 값은 cat에 저장됨 -->
            <option value="${cat}" <c:if test="${param.category == cat}">selected</c:if>>${cat}</option><!-- 반복될때마다 cat에 값이 저장되며 option에 값이 들어가게 되어 드롭박스를 생성 그리고 현재 선택된 카테고리 상태가 있을 경우 그것을 현상태로 유지 (<c:if test="${param.category == cat}">selected</c:if>)  -->
        </c:forEach>
    </select>
...
<form action="updateProducts.do" method="post" onsubmit="return validateForm();">
    <table>
        ...
        <%
            ...
            if(productList != null){ //productList가 있을 경우
                for (Product p : productList) { // productList를 하나씩 돌며 p에 넣으며 for문 실행
        %>
        ...
        <% } } else { %> <!-- productList에 값이 없을 경우  -->
        <tr><td colspan="6">照会された商品がありません。</td></tr><!-- 조회정보 없다고 경고창 발생 -->
        <% } %>
    </table>
    <br>
